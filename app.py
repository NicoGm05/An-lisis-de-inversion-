...
def _portfolio_as_df() -> pd.DataFrame:
    ...
    rows = []
    manual_yields = st.session_state.get("manual_yields", {})
    for ticker, info in st.session_state.portfolio.items():
        # Fetch manual yield for fixed income instruments
        annual_yield_manual = None
        if info.get("main_category", "") == "Renta Fija" and info.get("requires_manual_data", False):
            y = manual_yields.get(ticker, None)
            annual_yield_manual = y
        rows.append({
            "ticker": ticker,
            ...
            "annual_yield_manual": annual_yield_manual,
            "notes": info.get("notes", ""),
        })
    df = pd.DataFrame(rows)
    return df

def render_selected_portfolio(port_df: pd.DataFrame, capital: float, key_suffix: str = ""):
    st.markdown('<div class="sub-header">PORTAFOLIO SELECCIONADO</div>', unsafe_allow_html=True)
    if port_df.empty:
        st.info("Aún no has agregado activos al portafolio.")
        return

    display_cols = ["ticker", "name", "main_category", "sector", "region", "currency", "weight_pct", "monto_mxn", "annual_yield_manual", "status", "data_source"]
    col_labels = ["Ticker", "Nombre", "Tipo", "Sector", "Región", "Moneda", "Peso (%)", "Monto (MXN)", "Rend. Anual Manual", "Estatus", "Fuente"]
    
    display_df = port_df.copy()
    
    # some columns might not exist if empty or corrupted, safe fallback needed:
    ...
    display_df["Peso (%)"] = display_df["Peso (%)"].apply(lambda x: f"{x:.2f}%" if isinstance(x, (int, float)) else x)
    display_df["Monto (MXN)"] = display_df["Monto (MXN)"].apply(lambda x: f"${x:,.0f}" if isinstance(x, (int, float)) else x)
    ...
    def fmt_yield(val):
        ...
        if isinstance(val, (int, float)):
            return f"{val:.2f}%"
        return str(val) if str(val) else "N/D"
    display_df["Rend. Anual Manual"] = display_df["Rend. Anual Manual"].apply(fmt_yield)
    
    st.dataframe(display_df, use_container_width=True, hide_index=True)

    # Show FX and inflation notes for any USD or UDIBONOS assets in portfolio
    fi_assets = port_df[port_df["main_category"] == "Renta Fija"] if "main_category" in port_df.columns else pd.DataFrame()
    if not fi_assets.empty:
        usd_assets = fi_assets[fi_assets["currency"] == "USD"]["ticker"].tolist()
        udibono_assets = fi_assets[fi_assets["ticker"].str.contains("UDIBONO", na=False)]["ticker"].tolist()
        if usd_assets:
            st.markdown(
                f'<div class="alert-warn">⚠ Instrumentos en USD en el portafolio: {", ".join(usd_assets)}. '
                'Incluyen riesgo cambiario MXN/USD.</div>',
                unsafe_allow_html=True,
            )
        if udibono_assets:
            st.markdown(
                '<div class="alert-warn">⚠ UDIBONOS_10Y: Rendimiento real = 4.20% + inflación. '
                'No se convierte a tasa nominal sin inflación esperada capturada.</div>',
                unsafe_allow_html=True,
            )
    
    total_w = port_df["weight"].sum() if "weight" in port_df.columns else 0.0
    rv_w = port_df[port_df["main_category"] == "Renta Variable"]["weight"].sum() if "main_category" in port_df.columns else 0.0
    rf_w = port_df[port_df["main_category"] == "Renta Fija"]["weight"].sum() if "main_category" in port_df.columns else 0.0
    unassigned = max(0.0, 1.0 - total_w)
    
    col1, col2, col3, col4, col5 = st.columns(5)
    col1.metric("Total asignado", f"{total_w*100:.2f}%")
    col2.metric("Total RV", f"{rv_w*100:.2f}%")
    col3.metric("Total RF", f"{rf_w*100:.2f}%")
    col4.metric("Sin asignar", f"{unassigned*100:.2f}%")
    col5.metric("N° Activos", f"{len(port_df)}")
    
    csv = port_df.to_csv(index=False).encode("utf-8")
    st.download_button("⬇ Descargar CSV", csv, f"portafolio_seleccionado_{st.session_state.portfolio_name}.csv", "text/csv", key=f"dl_csv_sel_{key_suffix}")
...
with st.sidebar:
    ...
    # HTML Export
    st.markdown('<div class="sub-header">EXPORTAR REPORTE</div>', unsafe_allow_html=True)
    if st.button("📄 Generar HTML", use_container_width=True):
        with st.spinner("Generando reporte..."):
            port_df = _portfolio_as_df()
            wdict = {t: info.get("weight", 0) for t, info in st.session_state.portfolio.items() if not info.get("requires_manual_data", False)}
            
            m_val = None
            if PRICES_LOADED and not _port_prices.empty:
                m_val = full_portfolio_metrics(
                    _port_prices, wdict,
            ...
            rules = validate_portfolio(st.session_state.portfolio, universe_df, config, metrics=m_val)
            val_df = pd.DataFrame(rules)
            fund_df = st.session_state.get("_fundamentals", pd.DataFrame())
            
            figures = {}
            if not port_df.empty:
                figures["Composición por Tipo"] = chart_composition_type(port_df)
                figures["Composición por Sector"] = chart_composition_sector(port_df)
                figures["Composición por Región"] = chart_composition_region(port_df)
                
            if PRICES_LOADED and not _port_prices.empty:
                port_val = None
                if wdict:
                    port_val = calc_portfolio_value(_port_prices, wdict, st.session_state.capital)
                bench_val = None
                if not _bench_prices.empty and st.session_state.benchmark in _bench_prices.columns:
                    bench_val = _bench_prices[st.session_state.benchmark]
                figures["Crecimiento $1,000,000 MXN"] = chart_historical_performance(port_val, bench_val, capital=st.session_state.capital)
                
                port_ret = calc_portfolio_returns(_port_prices, wdict) if wdict else None
                figures["Drawdown"] = chart_drawdown(port_ret)
                
                corr = calc_correlation_matrix(_port_prices, wdict)
                figures["Correlaciones"] = chart_correlation_heatmap(corr)
                
            html_str = export_html_report(
                st.session_state.portfolio_name,
                port_df, val_df, m_val or {}, figures, fund_df, st.session_state.capital
            )
            now_dt = datetime.now().strftime("%Y%m%d_%H%M")
            fname_clean = "".join(c for c in st.session_state.portfolio_name if c.isalnum() or c in (" ", "-", "_")).replace(" ", "_")
            fname = f"actinver_portfolio_report_{fname_clean}_{now_dt}.html"
            
            st.session_state["_html_report"] = html_str
            st.session_state["_html_name"] = fname
        st.success("HTML generado, listo para descargar.")

    if "_html_report" in st.session_state:
        st.download_button(
            "⬇ Descargar resumen HTML",
            data=st.session_state["_html_report"].encode("utf-8"),
            file_name=st.session_state["_html_name"],
            mime="text/html",
...
with tabs[0]:
    ...
    st.markdown(
        "*Los instrumentos de renta fija son manuales. La tasa/rendimiento anual fue capturada por el usuario. "
        "No se inventan datos históricos. Las métricas cuantitativas se calculan solo con activos con precios disponibles.*"
    )
    st.markdown(
        '<div class="alert-info">ℹ <b>Cobertura de datos históricos:</b> '
        'Los instrumentos manuales de renta fija no se incluyen en series históricas. '
        'El rendimiento anual mostrado es el dato capturado por el usuario. '
        'Las métricas de volatilidad, VaR y correlación aplican solo a activos con precios históricos disponibles.</div>',
        unsafe_allow_html=True,
    )

    # Only show the 6 active manual RF instruments (status != "Revisar / Opcional")
    rf_active = universe_df[
        (universe_df["main_category"] == "Renta Fija") &
        (~universe_df["status"].str.contains("Opcional", na=False))
    ].copy()

    # Pre-defined rates from manual_fixed_income.csv for pre-population
    _MANUAL_RATES_DEFAULT = {
        "CETES_1Y": 7.17,
        "CETES_2Y": 8.50,
        "BONOS_MX_10Y": 9.20,
        "UDIBONOS_10Y": 4.20,
    ...
    # Load manual yields from session — pre-populate with defaults if not set
    if "manual_yields" not in st.session_state:
        st.session_state.manual_yields = {}
    for k, v in _MANUAL_RATES_DEFAULT.items():
        if k not in st.session_state.manual_yields:
            st.session_state.manual_yields[k] = v
    # Also load any values from manual_fixed_income.csv
    for _, row in manual_fi_df.iterrows():
        key = str(row.get("instrument_key", ""))
        y = row.get("annual_yield", None)
        if key not in st.session_state.manual_yields and pd.notna(y) and y != "":
            st.session_state.manual_yields[key] = float(y)

    rf_groups = [
        ("📌 CETES (Gubernamental MX)", rf_active[rf_active["sector"] == "CETES"]),
        ("📌 Bonos México (Gubernamental MX)", rf_active[rf_active["sector"] == "Bonos México"]),
        ("📌 UDIBONOS (Protegido inflación)", rf_active[rf_active["sector"] == "UDIBONOS"]),
        ("📌 Bonos USA (Gubernamental EE.UU.)", rf_active[rf_active["sector"] == "Bonos USA"]),
    ]

    # Special warnings per ticker
    _FX_WARNING_TICKERS = {"TBILLS_US_1Y", "TBONDS_US_30Y"}
    _UDIBONOS_TICKERS = {"UDIBONOS_10Y"}
    ...
    for group_label, group_df in rf_groups:
        ...
        with st.expander(
            f"**{group_label}** — {group_weight*100:.1f}% asignado ({len(group_df)} instrumentos)",
            expanded=False,
        ):
            for _, asset_row in group_df.iterrows():
                ...
                with col_name:
                    ...
                    default_rate = _MANUAL_RATES_DEFAULT.get(ticker, None)
                    if ticker in _UDIBONOS_TICKERS:
                        rate_display = f"4.20% + inflación"
                    elif default_rate is not None:
                        rate_display = f"{default_rate:.2f}%"
                    else:
                        rate_display = "Manual"
                    st.markdown(
                        ...
                        f'<br><span style="font-size:10px;color:#f0a500;">Rendimiento: {rate_display}</span>',
                        unsafe_allow_html=True,
                    )
                    # Inline warnings
                    if ticker in _FX_WARNING_TICKERS:
                        st.markdown(
                            '<div class="alert-warn" style="padding:4px 8px;font-size:11px;margin:2px 0;">'
                            '⚠ Instrumento en USD: incluye riesgo cambiario MXN/USD</div>',
                            unsafe_allow_html=True,
                        )
                    if ticker in _UDIBONOS_TICKERS:
                        st.markdown(
                            '<div class="alert-warn" style="padding:4px 8px;font-size:11px;margin:2px 0;">'
                            '⚠ Rendimiento real: 4.20% + inflación. No se convierte a tasa nominal sin inflación esperada capturada.</div>',
                            unsafe_allow_html=True,
                        )
                with col_w:
                    if checked or in_portfolio:
                ...
                with col_rate:
                    if is_manual:
                        if ticker in _UDIBONOS_TICKERS:
                            # UDIBONOS: show special field
                            current_rate = st.session_state.manual_yields.get(ticker, 4.20)
                            rate_input = st.number_input(
                                "Tasa real % + inflación",
                                min_value=0.0, max_value=100.0,
                                value=float(current_rate) if current_rate is not None else 4.20,
                                step=0.01, format="%.4f",
                                key=f"rate_{ticker}",
                                help="Rendimiento REAL. Se muestra como X% + inflación. Fuente: Banxico.",
                            )
                            st.session_state.manual_yields[ticker] = rate_input
                            st.caption("📌 Real + inflación")
                        elif checked or in_portfolio:
                            default_r = _MANUAL_RATES_DEFAULT.get(ticker, 0.0)
                            current_rate = st.session_state.manual_yields.get(ticker, default_r)
                            rate_input = st.number_input(
                                "Tasa anual % (Manual)",
                                min_value=0.0, max_value=100.0,
                                value=float(current_rate) if current_rate is not None else default_r,
                                step=0.01, format="%.4f",
                                key=f"rate_{ticker}",
                                help="Dato capturado manualmente por el usuario. Fuente: Banxico / U.S. Treasury / Actinver.",
                            )
                            st.session_state.manual_yields[ticker] = rate_input
                            st.caption("📌 Manual")
                        else:
                            default_r = _MANUAL_RATES_DEFAULT.get(ticker, 0.0)
                            st.markdown(
                                f'<span style="font-size:11px;color:#6b7a8d;">Tasa: {default_r:.2f}%</span>',
                                unsafe_allow_html=True,
                            )
                    else:
    ...
    st.markdown("---")
    render_selected_portfolio(_portfolio_as_df(), st.session_state.capital, key_suffix="cons")
...
with tabs[1]:
    ...
    if not PRICES_LOADED or _port_prices.empty:
        ...
        # Monto por activo
        st.markdown("---")
        render_selected_portfolio(_portfolio_as_df(), st.session_state.capital, key_suffix="res")
    ...
    if st.button("📊 Generar comparación", type="primary"):
        ...
        for pf_name in compare_sel:
            ...
            if PRICES_LOADED and not _port_prices.empty:
                ...
                row.update({
                    "Rendim. Anualiz.": format_pct(m_comp["annualized_return"]),
                    "CAGR": format_pct(m_comp["cagr"]),
                    "Volatilidad": format_pct(m_comp["volatility"]),
                    "Sharpe": format_num(m_comp["sharpe"], 3),
                    "Sortino": format_num(m_comp["sortino"], 3),
                    "Treynor": format_num(m_comp["treynor"], 3) if m_comp["treynor"] else "N/D",
                    "VaR 95%": format_pct(m_comp["var_95"]),
                    "Max Drawdown": format_pct(m_comp["max_drawdown"]),
                    "Beta": format_num(m_comp["beta"], 3),
                    "Alpha": format_pct(m_comp["alpha"]),
                })

            # Validation check
            rules = validate_portfolio(pf, universe_df, config, metrics=m_comp)
